...
_SCHEME_REQUEST_PATTERNS = [
    r'\bgive\s*(?:me\s+)?(?:the\s+)?schemes?\b',
    r'\bshow\s*(?:me\s+)?(?:the\s+)?schemes?\b',
    r'\blist\s*(?:of\s+)?schemes?\b',
    r'\bfind\s*(?:me\s+)?schemes?\b',
    r'\bsearch\s*(?:for\s+)?schemes?\b',
    r'\bwhat\s+schemes?\b', r'\bwhich\s+schemes?\b',
    r'\bschemes?\s+(?:available|for\s+me|do\s+you\s+have|i\s+(?:can|could)\s+get)\b',
    r'\beligible\s+(?:for\s+)?(?:which|what|any)\b',
    r'\bam\s+i\s+eligible\b',
    r'\bshow\s+(?:all|my)\s+(?:schemes?|results?|scholarships?)\b',
    ...
    r'\brecommend\s+(?:me\s+)?(?:some\s+)?schemes?\b',
    r'\bgive\s+(?:me\s+)?(?:a\s+)?list\b',
    r'\ball\s+(?:matching\s+)?schemes?\b',
    r'\bshow\s+results?\b',
]
